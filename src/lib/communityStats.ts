 ...
 * 仅在生产环境上报，开发/测试环境不上报。
 */
export async function reportFarmingCharacters(characters: CharacterReport[], server: Server = "cn"): Promise<void> {
  if (!supabase) return;
  // 开发环境不上报到数据库
...
// 按 server 分别缓存
const _cache: Partial<Record<Server, { result: CommunityResult; ts: number }>> = {};
const CACHE_TTL = 60 * 60 * 1000; // 1 小时内不重复请求（服务端也是每小时刷新）

// community_cache 表中 cn=1, gl=2
const CACHE_ID: Record<Server, number> = { cn: 1, gl: 2 };
 ...
 */
export async function fetchCommunityTop10(server: Server = "cn"): Promise<CommunityResult | null> {
  if (!supabase) return null;
  const cached = _cache[server];
  ...
  if (!cacheErr && cacheRow?.data) {
    ...
    _cache[server] = { result, ts: Date.now() };
    return result;
  }
